...
export class NpmFixer extends BaseFixer {
  ...
  async applyFix(
    ...
    for (const issue of fixableIssues) {
      ...
      if (info.targetVersion) {
        upgrades.push(`${info.packageName}@${info.targetVersion}`);
        fixedIssues.push(issue);
      } else {
        upgrades.push(info.packageName);
        fixedIssues.push(issue);
      }
    }
    ...
    logger.info(`[npm] Upgrading packages: ${upgrades.join(', ')}`);

    if (dryRun) {
      logger.info('[npm] Dry run — skipping actual npm install');
      return this.createSuccessResult(fixedIssues, upgrades.map((p) => `Would upgrade: ${p}`));
    }

    try {
      await this.runCommand('npm', ['install', ...upgrades, '--save'], workingDirectory);
      ...
      logger.error(`[npm] Fix failed: ${msg}`);
