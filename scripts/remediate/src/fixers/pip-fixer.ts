...
export class PipFixer extends BaseFixer {
  ...
  async applyFix(
    ...
    logger.info(`[pip] Upgrading packages: ${upgrades.join(', ')}`);

    if (dryRun) {
      logger.info('[pip] Dry run — skipping actual pip install');
    ...
    try {
      ...
      logger.error(`[pip] Fix failed: ${msg}`);
