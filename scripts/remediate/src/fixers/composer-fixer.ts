...
export class ComposerFixer extends BaseFixer {
  readonly packageManager = 'composer' as const;
  ...
  async applyFix(
    ...
    if (fixableIssues.length === 0) {
      return this.createErrorResult(issues, 'No fixable Composer issues found');
    }
    ...
    logger.info(`[composer] Upgrading packages: ${upgrades.join(', ')}`);
    ...
    try {
      await this.runCommand('composer', ['require', ...upgrades], workingDirectory);
      ...
      logger.error(`[composer] Fix failed: ${msg}`);
