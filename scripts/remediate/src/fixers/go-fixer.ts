...
export class GoFixer extends BaseFixer {
  ...
  async applyFix(
    ...
    if (fixableIssues.length === 0) {
      return this.createErrorResult(issues, 'No fixable Go issues found');
    }
    ...
    for (const issue of fixableIssues) {
      ...
      const pkg = info.targetVersion
        ? `${info.packageName}@v${info.targetVersion}`
        : `${info.packageName}@latest`;
      upgrades.push(pkg);
    ...
    logger.info(`[go] Upgrading packages: ${upgrades.join(', ')}`);
    ...
    try {
      for (const pkg of upgrades) {
        await this.runCommand('go', ['get', pkg], workingDirectory);
      }
      await this.runCommand('go', ['mod', 'tidy'], workingDirectory);
      ...
      logger.error(`[go] Fix failed: ${msg}`);
