...
export class YarnFixer extends BaseFixer {
  readonly packageManager = 'yarn' as const;
  ...
  async applyFix(
    ...
    logger.info(`[yarn] Upgrading packages: ${upgrades.join(', ')}`);

    if (dryRun) {
      logger.info('[yarn] Dry run — skipping actual yarn upgrade');
      return this.createSuccessResult(fixedIssues, upgrades.map((p) => `Would upgrade: ${p}`));
    }

    try {
      await this.runCommand('yarn', ['upgrade', ...upgrades], workingDirectory);
      ...
      logger.error(`[yarn] Fix failed: ${msg}`);
